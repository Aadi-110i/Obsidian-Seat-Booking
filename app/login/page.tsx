            ...
            <div className="fade-in" style={{ width: '100%', maxWidth: 440, position: 'relative', zIndex: 10 }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: 32 }}>
                    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                        <div style={{ width: 44, height: 44, background: 'var(--text-primary)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', flexShrink: 0 }}>
                            <div style={{ width: 22, height: 22, borderRadius: 4, background: 'var(--bg-primary)', transform: 'rotate(45deg)' }} />
                        </div>
                        <div>
                        <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-primary)' }}>Obsidian</div>
                            <div style={{ fontSize: 11, color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Seat Booking</div>
                        </div>
                    </div>
                    ...
                    <p style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
                        {mode === 'login' ? 'Sign in to manage your seat bookings' : 'Join Obsidian seat booking system'}
                    </p>
                </div>
                ...
                <div className="glass" style={{ padding: 32 }}>
                    {/* Tab toggle */}
                    <div style={{ display: 'flex', background: 'var(--bg-secondary)', borderRadius: 12, padding: 6, marginBottom: 24, border: '1px solid var(--border)' }}>
                        {(['login', 'register'] as Mode[]).map((m) => (
                            <button key={m} onClick={() => { setMode(m); setError(''); }} style={{ flex: 1, padding: '10px', fontSize: 14, fontWeight: 600, background: mode === m ? 'var(--accent)' : 'transparent', color: mode === m ? 'var(--bg-primary)' : 'var(--text-secondary)', border: 'none', borderRadius: 8, transition: 'all 0.2s', cursor: 'pointer' }}>
