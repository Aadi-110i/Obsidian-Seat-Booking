...
export async function POST(req: NextRequest) {
    try {
        ...
        // Fetch the seat and its bookings for the target date
        ...
        if (effectiveType === 'DESIGNATED') {
            ...
            if (!isFloatingUnlocked(targetDate)) {
                return NextResponse.json({ error: 'Floating seats unlock at 3:00 PM the previous working day (no same-day bookings)' }, { status: 403 });
            }
        }
