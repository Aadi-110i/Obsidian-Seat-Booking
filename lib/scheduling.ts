 ...
 * Floating seats for the next working day unlock at 3:00 PM the working day prior.
...
export function isFloatingUnlocked(targetDate: Date): boolean {
    ...
    // Same-day bookings or past bookings are not allowed
    if (target.getTime() <= today.getTime()) return false;

    // Floating seats unlock 1 working day prior at 3:00 PM.
    const dayOfWeek = getDay(target);
    if (dayOfWeek === 0 || dayOfWeek === 6) return false;

    let unlockDate = addDays(target, -1);
    if (dayOfWeek === 1) { // If target is Monday
        unlockDate = addDays(target, -3); // Unlock on Friday
    }

    const unlockTime = new Date(unlockDate);
    unlockTime.setHours(15, 0, 0, 0);

    return now.getTime() >= unlockTime.getTime();
}
